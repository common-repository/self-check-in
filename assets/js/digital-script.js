...
jQuery(document).ready(function($){
    ...
    jQuery(document).on('change', '.doc_img', function(event) {
        ...
        var allFiles = input.data('files') || [];
        
        // Concatenate old inputs with new ones
        var allFiles = allFiles.concat(Array.from(event.target.files));
        ...
        Array.from(fileList).forEach(function(file) {
            ...
            reader.onload = function(e) {
                ...
                previewHTML += '<span class="remove-btn">&times;</span>';
                previewHTML += '</div>';
    ...
    jQuery('#updated_booking_id').on('input', function(){
        ...
        if(element.data('id') == id){
            ...
            jQuery('button[name="wpsci_update_overview"]').prop('disabled', false);
            return;
        }
        jQuery('button[name="wpsci_update_overview"]').prop('disabled', true);
        jQuery.ajax({
            url: wpsci.ajaxurl,
            method: 'POST',
            data: {
                action: 'check_booking_id',
                verify_nonce: wpsci.verify_nonce,
                booking_id: id,
            },
